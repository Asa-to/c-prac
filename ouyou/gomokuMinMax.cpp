...
#define MINMAX_SEARCH_AI 5    // minmax探索のAI

char namesAI[][16] = {
    "", "", "FirstFreeAI", "RandomAI", "OneMoveSearchAI", "MinMaxSearchAI"};
...
int blackClosedFour;  // 黒い石を四つ並びの数、白い石のブロック (W B B B B *)
                      // か盤の端にブロック (E B B B B *)
int blackOpenThree;  // 黒い石を三つ並びの数、ブロックなし： * B B B *
int blackClosedThree;  // 黒い石を三つ並びの数、白い石のブロック (W B B B *)
                       // か盤の端にブロック (E B B B *)
int blackOpenTwo;  // 黒い石を二つ並びの数、ブロックなし： * B B *
int blackClosedTwo;  // 黒い石を二つ並びの数、白い石のブロック (W B B *)
                     // か盤の端にブロック (E B B *)
...
// 石の連結の評価値（調整していないのでもっと良い値があるはず）
#define CLOSED_FOUR_VALUE 30
#define OPEN_THREE_VALUE 50
#define CLOSED_THREE_VALUE 20
#define OPEN_TWO_VALUE 10
#define CLOSED_TWO_VALUE 5
...
int main() {
    ...
    while (1) {
        ...
        printf(
            "=============================================================\n");
        printf(
            "This is a very weak Gomoku program made by Reijer Grimbergen.\n");
        printf(
            "=============================================================\n");
        printf("Main menu:\n");
        ...
        switch (userSelection) {
            ...
            case '4':
                ...
                if (blackPlayer && whitePlayer) {
                    ...
                    //scanf_s("%*c"); 				fopen_s(&dataFile, dataFileName, "w");
                    if (dataFile == NULL)
                    ...
                    for (i = 1; i <= numberOfGames; i++) {
                        ...
                        if ((gameResult == BLACK_WIN &&
                            ...
                            firstAIWins++;
                                  ...
                                  blackPlayer == secondAI) ||
                                 (gameResult == WHITE_WIN &&
                                  whitePlayer == secondAI))
                            secondAIWins++;
                        else if (gameResult == DRAW)
                            drawNumber++;
                        // 黒と白を入れ替えて、次の対局をやる
                        if (i != numberOfGames) {
                            printf("\nCurrent score: %s - %s %d-%d(%d)\n\n",
                                   namesAI[firstAI], namesAI[secondAI],
                                   firstAIWins, secondAIWins, drawNumber);
                            if (saveData)
                                fprintf(
                                    dataFile,
                                    "\nCurrent Score: %s - %s %d-%d(%d)\n\n",
                    ...
                    printf("\nFinal result: %s - %s %d-%d(%d)\n\n",
                           namesAI[firstAI], namesAI[secondAI], firstAIWins,
                           secondAIWins, drawNumber);
                    if (saveData) {
                        fprintf(dataFile,
                                "\nFinal result: %s - %s %d-%d(%d)\n\n",
                                namesAI[firstAI], namesAI[secondAI],
            ...
            default:
                printf(
                    "Illegal input: Please select one of the menu options.\n");
                break;
        }
    }
    return 0;
...
void makeBoardEmpty() {
    ...
    for (x = 1; x <= BOARDSIZE; x++)
        for (y = 1; y <= BOARDSIZE; y++) gomokuBoard[x][y] = EMPTY_SQUARE;
}
...
int playGomokuGame() {
    ...
    while (gameResult == UNFINISHED) {
        if ((toMove == BLACK && blackPlayer != HUMAN) ||
            ...
            if (saveData) fprintf(dataFile, "Thinking about my move...\n");
        ...
        // 更新した局面を表示
        showBoard();
        if (saveData) saveBoardToFile(dataFile);
        ...
        if (fiveInRowCheck(nextMoveX, nextMoveY, toMove)) {
            ...
            break;
                   ...
                   (BOARDSIZE * BOARDSIZE))  // 盤はいっぱいになったかどうか
            gameResult = DRAW;
...
int fiveInRowCheck(int lastX, int lastY, int color) {
    ...
    // 横の五目並べチェック:(lastx,
    // lasty)座標からcolorと同じ色の石の連結を右と左にあわせて数える
    if (lastX > 1) {
    ...
    if (connectNo >= 5) return 1;  // 横の五目並べ見つけた

    // 縦の五目並べチェック:(lastx,
    // lasty)座標からcolorと同じ色の石の連結を上と下にあわせて数える
    connectNo = 1;
    if (lastY > 1) {
    ...
    if (connectNo >= 5) return 1;  // 縦の五目並べ見つけた

    // 斜め（北西－南東）の五目並べチェック:(lastx,
    // lasty)座標からcolorと同じ色の石の連結を北西と南東にあわせて数える
    connectNo = 1;
    if (lastX > 1 && lastY > 1) {
        for (x = lastX - 1, y = lastY - 1; gomokuBoard[x][y] == color;
             x--, y--) {
            connectNo++;
            if (x == 1 || y == 1) break;
        }
    ...
    if (lastX < BOARDSIZE && lastY < BOARDSIZE) {
        for (x = lastX + 1, y = lastY + 1; gomokuBoard[x][y] == color;
             x++, y++) {
            connectNo++;
            if (x == BOARDSIZE || y == BOARDSIZE) break;
        }
    }
    if (connectNo >= 5) return 1;  // 斜め（北西－南東）の五目並べ見つけた

    // 斜め（北東－南西）の五目並べチェック:(lastx,
    // lasty)座標からcolorと同じ色の石の連結を北東と南西にあわせて数える
    connectNo = 1;
    if (lastX < BOARDSIZE && lastY > 1) {
        for (x = lastX + 1, y = lastY - 1; gomokuBoard[x][y] == color;
             x++, y--) {
            connectNo++;
            if (x == BOARDSIZE || y == 1) break;
        }
    }
    if (lastX > 1 && lastY < BOARDSIZE) {
        for (x = lastX - 1, y = lastY + 1; gomokuBoard[x][y] == color;
             x--, y++) {
            connectNo++;
            if (x == 1 || y == BOARDSIZE) break;
        }
    }
    if (connectNo >= 5) return 1;  // 斜め（北東－南西）の五目並べ見つけた
    ...
    while (!legalMove) {
        ...
        while (!legalX) {
            ...
            if (nextMoveX >= 1 && nextMoveX <= BOARDSIZE)
                ...
                printf(
                    "The x-coordinate should be a number between 1 and 15.\n");
                nextMoveX = 0;
            }
        }
        // 正しいy軸の獲得まで繰り返す
        while (!legalY) {
            ...
            if (nextMoveY >= 1 && nextMoveY <= BOARDSIZE)
                ...
                printf(
                    "The y-coordinate should be a number between 1 and 15.\n");
                nextMoveY = 0;
            }
        }
        // 選んだマスは空きマスかどうかを確認
...
int decideComputerMove(int toMove) {
    ...
    switch (which_ai) {
        ...
        case MINMAX_SEARCH_AI:
            score = minMaxSearch(0, rootToMove);
            printf("Best move evaluation: %d\n", score);
            if (saveData)
                fprintf(dataFile, "Best move evaluation: %d\n", score);
            break;
    ...
    for (i = 1; i <= legalMoveNo; i++) {
        ...
        if (gomokuBoard[x][y] == EMPTY_SQUARE) {
            ...
            if (fiveInRowCheck(x, y, toMove)) {
                ...
                // 石を置いた局面を評価
                if (toMove == BLACK)
                    eval = evaluate(rootToMove, WHITE, 1);
                else
                    eval = evaluate(rootToMove, BLACK, 1);
...
// Minmax探索
//
int minMaxSearch(int depth, int toMove) {
    int score, eval;
    int x, y;
    int i, legalMoveNo;
    Move allMoves[BOARDSIZE * BOARDSIZE + 1];
    ...
    // 最大深さになったら局面を評価する
    if (depth == MAXDEPTH) return evaluate(rootToMove, toMove, depth);
    ...
    for (i = 1; i <= legalMoveNo; i++) {
        ...
        if (gomokuBoard[x][y] == EMPTY_SQUARE) {
            ...
            if (fiveInRowCheck(x, y, toMove)) {
                ...
                // minmax探索を再帰的に呼ぶ
                eval = minMaxSearch(depth + 1, flip(toMove));
    ...
    for (x = 1; x <= BOARDSIZE; x++) {
        for (y = 1; y <= BOARDSIZE; y++) {
            if (gomokuBoard[x][y] == BLACK_STONE) {
                ...
                if (x == 1 || y == BOARDSIZE ||
                    gomokuBoard[x - 1][y + 1] == WHITE_STONE)
                    blocked = 1;
                else {
                    blocked = 0;
                    emptySqCon++;
                ...
                // 石の連結を数える
                for (countX = x + 1, countY = y - 1;
                     countX <= BOARDSIZE && countY >= 1; countX++, countY--) {
                ...
                if (blocked <= 1 && connectNo > 1) {
                    for (; countX <= BOARDSIZE && countY >= 1 &&
                           gomokuBoard[countX][countY] == EMPTY_SQUARE;
                         countX++, countY--)
                        emptySqCon++;
                }
                // 石の連結の種類の数を更新
                ...
                // 石の連結を数える
                for (countX = x + 1, countY = y + 1;
                     countX <= BOARDSIZE && countY <= BOARDSIZE;
                     countX++, countY++) {
                    if (gomokuBoard[countX][countY] == BLACK_STONE) {
                        connectNo++;
                    } else if (gomokuBoard[countX][countY] == WHITE_STONE) {
                ...
                // どこまで伸ばせるか
                if (blocked <= 1 && connectNo > 1) {
                    for (; countX <= BOARDSIZE && countY <= BOARDSIZE &&
                           gomokuBoard[countX][countY] == EMPTY_SQUARE;
                         countX++, countY++)
                        emptySqCon++;
                }
                ...
                // 石の連結は盤の端か相手の石にブロックされている
                if (x == 1 || y == BOARDSIZE ||
                    gomokuBoard[x - 1][y + 1] == BLACK_STONE)
                    blocked = 1;
                else {
                    blocked = 0;
                    emptySqCon++;
